...
from Reports.forms import (CashRegisterReportForm, ProductsReportForm,
                           SalesReportForm)
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if worksheet is not None:
                ...
                worksheet.append(
                    [
                        "ID", "Data abertura", "Data fechamento",
                        "Saldo", "QTD Vendas"
                    ]
                )

            for cashregister in cashregisters:
                open_date = cashregister.open_date.replace(
                    tzinfo=None) if cashregister.open_date is not None \
                    else cashregister.open_date
                close_date = cashregister.close_date.replace(
                    tzinfo=None) if cashregister.close_date is not None \
                    else cashregister.close_date
            ...
            response['Content-Disposition'] = \
                'attachment;filename=Relatorio_de_Caixa.xlsx'
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if worksheet is not None:
                ...
                worksheet.append(
                    ["ID Venda", "Data", "Cliente", "Tipo de Venda",
                     "Itens", "Total (R$)", "Tipo de Pagamento"]
                )
            ...
            for sale in sales:
                ...
                if sale_type == "Produto":
                    items = ", ".join(
                        [product.product_name for product in
                         sale.products.all()]
                    )
                else:
                    if sale.schedule is not None:
                        items = ", ".join(
                            [service.service_name for service in
                             sale.schedule.services.all()]
                        )
            ...
            response['Content-Disposition'] = \
                'attachment; filename=Relatorio_de_Vendas.xlsx'
    ...
    def post(self, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if worksheet is not None:
                ...
                worksheet.append(
                    ["ID Produto", "Nome", "Categoria", "Preço (R$)",
                     "Status", "Mostrar na Vitrine", "Data de Criação"]
                )
            ...
            for product in products:
                ...
                if worksheet is not None:
                    worksheet.append([
                        ...
                        product.product_category.category_name if
                        product.product_category else "Sem Categoria",
                        product.price,
                        status,
                        showcase,
            ...
            response['Content-Disposition'] = \
                'attachment; filename=Relatorio_de_Produtos.xlsx'
